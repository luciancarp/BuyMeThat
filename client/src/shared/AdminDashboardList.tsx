...
import ListElFlagged from './ListElementFlagged';
import { map } from 'lodash';

class AdminDashboardList extends React.Component<any,any>{
  ...
  var listPropsAwaiting = [];
    ...
    var listPropsFlagged = [];
    for (var i=0; i < this.props.flaggedOrders.length; i++) {
    var elementProps: ListElProp = {
      id: this.props.flaggedOrders[i].id,
      nameUser: this.props.flaggedOrders[i].firstname + " " + this.props.flaggedOrders[i].lastname,
      itemName: this.props.flaggedOrders[i].name,
      itemDescription: this.props.flaggedOrders[i].description,
      itemStatus: this.props.flaggedOrders[i].status,
      itemTime: this.props.flaggedOrders[i].time,
      provider: this.props.flaggedOrders[i].providername,
      url: this.props.flaggedOrders[i].url
    };

    var elPropsContainer: ListElPropContainer = {
    elProps: elementProps
    };
    listPropsFlagged.push(elPropsContainer);
    };
      ...
      <MuiThemeProvider muiTheme={materialUiTheme}>
        <WrappedList>
          <div className={'ListItems'}>
            <List>
              <RaisedButton label="New Request" secondary={true} onClick={() => {location.href = '/NeederNewRequest'}}/>
              <RaisedButton label="User Management" secondary={true} style={{marginLeft:10}} onClick={() => {location.href = '/UserList'}}/>
              <Subheader inset={true}>Flagged Orders</Subheader>
              <div className={'ListEl'}>
              {map(listPropsFlagged, (id: ListElPropContainer) => <ListElFlagged key={id.elProps.id} elProps={id.elProps} onUpdateStatus={this.props.onUpdateStatus} onSubmitNote={this.props.onSubmitNote} getNotes={this.props.getNotes}/>)}
              </div>
              <br/>
              ...
              <div className={'ListEl'}>
              {map(listPropsManaged, (id: ListElPropContainer) => <ListElManaged key={id.elProps.id} elProps={id.elProps} onUpdateStatus={this.props.onUpdateStatus} onSubmitNote={this.props.onSubmitNote} getNotes={this.props.getNotes}/>)}
              </div>
              <br/>
